  ...
  return (
    <div className="flex justify-center w-full gap-10 mt-10">
        ...
        <div className="w-full h-[80%] flex justify-center">
          <section className="w-full  p-5 border-2 border-gray-200 rounded-xl flex flex-col gap-4">
            <div className="w-full flex flex-row justify-between">
              <div className="text-sm text-gray-500">날짜를 누르면 해당일의 상세내역을 볼 수 있어요.</div>
            </div>
                  ...
                  <div className="flex w-full h-full" key={`week${i}`}>
                    <div className="flex w-full">
                      {week.map((v, j) => {
                        ...
                        return <CalendarCell key={`${i}-${j}`} store_id={store_id} isCurrentMonth={isCurrentMonth} isToday={isToday} date={v} />
                      })}
                    </div>
                  </div>
                )
      ...
      <div className="flex h-full flex-col justify-center gap-2">
        <Button className="bg-white border border-gray-200 h-[250px]" onClick={dateHandler(addYears)} size="sm">
          <SlArrowUp className="text-black" size="10" />
        </Button>
        <Button className="bg-white border border-gray-200 h-[250px]" onClick={dateHandler(subYears)} size="sm">
